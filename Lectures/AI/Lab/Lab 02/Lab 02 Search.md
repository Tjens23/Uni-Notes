...
def INSERT_ALL(list, queue):
    for node in list:
        queue = INSERT(node, queue)  # insert each node in the list into the queue
    return queue  # return the updated queue
...
def REMOVE_FIRST(queue):
    if len(queue) == 0:  # if queue is empty
        ...
        del queue[0]  # remove the first node from the queue
        return node  # return the removed node
...
def successor_fn(state):  # Lookup list of successor states
    ...
    if location == 'A' and a_status == 'Dirty':
        successors.append(('A', 'Clean', b_status))
    elif location == 'B' and b_status == 'Dirty':
        successors.append((location, a_status, 'Clean'))
    else:
        successors.append((location, a_status, b_status))  # No action taken
    if location == 'B':
        successors.append(('A', a_status, b_status))

    elif location == 'A':
        successors.append(('B', a_status, b_status))
    return successors

INITIAL_STATE = ('A', 'Dirty', 'Dirty')
GOAL_STATE = ('A', 'Clean', 'Clean')  # Goal state is when both rooms are clean
