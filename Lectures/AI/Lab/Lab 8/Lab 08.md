...
class Variable(object):
    ...
    def calculate_marginal_probability(self):
        ...
        for assignment_value in self.assignments:
            probability_sum = 0
            for row in self.probability_table:
                conditional_probability = self.probability_table[row][self.assignments[assignment_value]]
                parents_join_probability = 1
                for i in range(len(row)-1):
                    parent_value = row[i]
                    parent_node = self.parents[i]
                    parents_join_probability *= parent_node.get_marginal_probability(parent_value)
                    probability_sum += conditional_probability * parents_join_probability
                assigment_index = self.assignments[assignment_value]
                self.marginal_probabilities[assigment_index] = conditional_probability
        self.ready = True
...
class BayesianNetwork(object):
    ...
    def get_joint_probability(self, values):
        ...
        for var_name, value in values.items():
            ...
            if not var.ready:
                raise ValueError("Variable {} is not ready, please calculate marginal probabilities first.".format(var_name))
            parents_values = self.sub_vals(var, values)
            res *= var.get_probability(value, parents_values)
        return res
