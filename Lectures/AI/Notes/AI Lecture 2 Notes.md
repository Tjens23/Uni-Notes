...
- **Description**: These agents use a large lookup table that maps percept sequences to actions.
- **Challenges**:
...
- **Example**: plaintext If the environment is dirty, then suck.
- **Limitations**:
...
- **Example**: plaintext function REFLEX-AGENT-WITH-STATE(percept) returns an action state ← UPDATE-STATE(state, action, percept, model) rule ← RULE-MATCH(state, rules) action ← RULE-ACTION[rule] return action
...
- **Functionality**:
    - They choose actions that maximize their expected utility, allowing for more nuanced decision-making compared to goal-based agents.

## Learning and Autonomous Agents  

- Learning agents utilize mechanisms to adapt and improve their decision-making process in unknown environments.
